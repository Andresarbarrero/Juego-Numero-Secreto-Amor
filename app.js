...
let maximosIntentos = 5; // Se crea una variable para el número máximo de intentos y dar una condición de salida
...
function verificarIntento() {
    ...
    if (numeroDeUsuario === numeroSecreto) { // Se crea una condición para comparar si el valor ingresado por el usuario es igual al número generado aleatoriamente
        asignarTextoElemento ('p', `¡Felicidades mi vida! ¡Adivinaste el número secreto en ${intentos} ${(intentos === 1) ? 'vez' : 'veces'}!¡JUEGA DE NUEVO!`); //TS y Operador ternario
        ...
        if (numeroDeUsuario > numeroSecreto) { // Se crea una condición para comparar si el valor ingresado por el usuario es mayor al número generado aleatoriamente
            asignarTextoElemento ('p', '¡El número secreto es menor cachetitos!'); // Se cambia el contenido de p por el mensaje que queremos
        } else {
            asignarTextoElemento ('p', '¡El número secreto es mayor mi bonita!'); // Se cambia el contenido de p por el mensaje que queremos
        }
        intentos++; // Se incrementa la variable intentos en 1 
        if (intentos > maximosIntentos) {
            asignarTextoElemento('p',`¡Lo siento amor!, se acabaron los intentos. El número secreto era ${numeroSecreto}. Puedes volver a intentarlo con un nuevo juego.`); 
...
function condicionesIniciales() {
    asignarTextoElemento ('h1', 'UN JUEGO PARA TI AMOR'); // Se llama a la función para que se ejecute (solo en eventos) y se cambie el contenido de h1 por el titulo que queremos
